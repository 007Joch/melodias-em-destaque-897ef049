      ...
      try {
        ...
        if (hostname === 'localhost' || hostname === '127.0.0.1') {
          ...
          if (clientParam) {
            ...
            // Verificar também se o email é testhabboo@gmail.com (admin configurado)
            const isUserAdmin = currentUser?.role === 'admin' || currentUser?.email === 'testhabboo@gmail.com';
            setIsAdminMode(isUserAdmin);
            console.log(`Ambiente local: modo admin ${isUserAdmin ? 'ativado' : 'desativado'} - usuário ${isUserAdmin ? 'é' : 'não é'} admin`, { currentUser });
          }
        } else {
          ...
          if (parts.length === 4 && parts[0] === 'app' && parts[1] === 'delliapp' && parts[2] === 'com' && parts[3] === 'br') {
            setSubdomain(null);
            // Ativar modo admin APENAS se usuário tem role 'admin' ou email específico
            const isUserAdmin = currentUser?.role === 'admin' || currentUser?.email === 'testhabboo@gmail.com';
            setIsAdminMode(isUserAdmin);
            console.log(`Produção: app.delliapp.com.br - modo admin ${isUserAdmin ? 'ativado' : 'desativado'} - usuário ${isUserAdmin ? 'é' : 'não é'} admin`, { currentUser });
          }
